...
export const SendEmailModal: React.FC<SendEmailModalProps> = ({
  ...
  }, [opened, initialTemplateId, initialProviderId]);

  // Handle template selection and mockup data generation
  React.useEffect(() => {
    if (form.values.templateId) {
      const template = templates.find(t => t.id === form.values.templateId);
      if (template) {
        // If templateData is empty or just default '{}', generate mockup data
        if (form.values.templateData === '{}') {
          const mockup = extractVariables(template);
          form.setFieldValue('templateData', JSON.stringify(mockup, null, 2));
        }
      }
    }
  }, [form.values.templateId, templates, form.values.templateData]);
